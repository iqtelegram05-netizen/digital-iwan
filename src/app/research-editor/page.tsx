...
// ========== Arabic Fonts Map ==========
const ARABIC_FONTS = [
  { name: 'Noto Naskh Arabic', label: 'نسخ عربي', style: "font-family: 'Noto Naskh Arabic', serif" },
  { name: 'Noto Serif Arabic', label: 'خط عربي', style: "font-family: 'Noto Serif Arabic', serif" },
  { name: 'Amiri', label: 'أميري', style: "font-family: 'Amiri', serif" },
  { name: 'Scheherazade New', label: 'شهرزاد', style: "font-family: 'Scheherazade New', serif" },
  { name: 'Lateef', label: 'لطيف', style: "font-family: 'Lateef', serif" },
  { name: 'Markazi Text', label: 'مركزي', style: "font-family: 'Markazi Text', serif" },
  { name: 'Aref Ruqaa', label: 'عارف رقعة', style: "font-family: 'Aref Ruqaa', serif" },
  { name: 'Katibeh', label: 'كاتبة', style: "font-family: 'Katibeh', serif" },
  { name: 'Mirza', label: 'ميرزا', style: "font-family: 'Mirza', serif" },
  { name: 'Noto Kufi Arabic', label: 'كوفي', style: "font-family: 'Noto Kufi Arabic', sans-serif" },
  { name: 'Reem Kufi', label: 'ريم كوفي', style: "font-family: 'Reem Kufi', sans-serif" },
  { name: 'Lalezar', label: 'لاله زار', style: "font-family: 'Lalezar', display" },
  { name: 'IBM Plex Sans Arabic', label: 'آي بي أم', style: "font-family: 'IBM Plex Sans Arabic', sans-serif" },
  { name: 'IBM Plex Serif Arabic', label: 'آي بي أم سيريف', style: "font-family: 'IBM Plex Serif Arabic', serif" },
];
...
const DEFAULT_SETTINGS_DARK: EditorSettings = {
  ...DEFAULT_SETTINGS,
  darkMode: true,
};
...
export default function ResearchEditorPage() {
  ...
  const addFootnote = () => {
    ...
    // Insert superscript reference in text
    const superscript = `<sup style="color:#0ea5e9;cursor:pointer;font-size:10px;" data-fn="${fn.id}" onclick="document.querySelector('[data-fn-panel]')?.classList.toggle('hidden')">${fn.number}</sup>`;
    document.execCommand('insertHTML', false, superscript);
  ...
  if (showList && !currentPaper) {
    return (
      <div className="min-h-screen" style={{ background: settings.darkMode ? '#0f172a' : '#f8fafc' }}>
        {/* Header */}
        <div className="sticky top-0 z-30 backdrop-blur-xl border-b" style={{
          background: settings.darkMode ? 'rgba(15,23,42,0.9)' : 'rgba(255,255,255,0.9)',
          borderColor: settings.darkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)'
        }}>
          <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold" style={{ color: settings.darkMode ? '#e2e8f0' : '#0f172a' }}>محرر البحوث</h1>
                <p className="text-[11px]" style={{ color: settings.darkMode ? '#94a3b8' : '#64748b' }}>فريق قلم كود — معالجة محلية بالكامل</p>
              </div>
            </div>
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{ background: bgColor }}>
      {/* ===== TOP TOOLBAR ===== */}
      <div className="shrink-0 border-b px-2 py-1.5 flex items-center gap-1 overflow-x-auto scrollbar-hide" style={{
        background: settings.darkMode ? 'rgba(15,23,42,0.95)' : 'rgba(255,255,255,0.95)',
        borderColor: settings.darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)',
        backdropFilter: 'blur(12px)',
      }}>
        {/* Back */}
        <button onClick={() => { setCurrentPaper(null); setShowList(true); }} className={btnClass} title="العودة">
          <ArrowRight className="w-4 h-4" />
        </button>

        <div className="w-px h-5 mx-1" style={{ background: settings.darkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }} />

        {/* Font Selector */}
        <div className="relative">
          <button onClick={() => { setShowFontDropdown(!showFontDropdown); setShowDecorDropdown(false); }} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-medium transition-colors ${showFontDropdown ? 'bg-primary/15 text-primary' : 'hover:bg-primary/10 text-foreground/70 hover:text-primary'}`}>
            <Type className="w-3.5 h-3.5" />
            <span className="max-w-[80px] truncate">{ARABIC_FONTS.find(f => f.name === settings.fontFamily)?.label || 'خط'}</span>
            <ChevronDown className="w-3 h-3" />
          </button>
          <AnimatePresence>
            {showFontDropdown && (
              <motion.div
                className="absolute top-full right-0 mt-1 w-48 rounded-xl border shadow-xl z-50 overflow-hidden max-h-64 overflow-y-auto"
              ...
              >
                {ARABIC_FONTS.map(font => (
                  <button
                    key={font.name}
                    onClick={() => { setSettings(s => ({ ...s, fontFamily: font.name })); setShowFontDropdown(false); }}
                    className="w-full text-right px-3 py-2 text-xs hover:bg-primary/10 transition-colors"
                    style={{ color: settings.darkMode ? '#e2e8f0' : '#1e293b', ...JSON.parse(font.style.replace(/'/g, '"').replace(/font-family:\s*/g, '"fontFamily": "').replace(/,\s*serif|,\s*sans-serif|,\s*display/g, '"').replace(/;?\s*$/, '"')) }}
                  >
                    {font.label}
                  </button>
                ))}
        ...
        <select
          ...
          className="h-7 w-14 text-[11px] rounded-lg border px-1"
        ...
        {[['justify', AlignJustify], ['right', AlignRight], ['center', AlignCenter], ['left', AlignLeft]].map(([align, Icon]) => (
          <button key={align} onClick={() => setSettings(s => ({ ...s, textAlign: align }))} className={`${settings.textAlign === align ? activeBtnClass : btnClass}`}>
        ...
        <select
          ...
          className="h-7 w-16 text-[11px] rounded-lg border px-1"
          style={{ background: settings.darkMode ? 'rgba(30,41,59,0.8)' : '#f8fafc', borderColor: settings.darkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)', color: textColor }}
        >
        ...
        <div className="relative">
          <button onClick={() => { setShowDecorDropdown(!showDecorDropdown); setShowFontDropdown(false); }} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] transition-colors ${showDecorDropdown ? 'bg-primary/15 text-primary' : 'hover:bg-primary/10 text-foreground/70 hover:text-primary'}`}>
          ...
          <AnimatePresence>
            {showDecorDropdown && (
              <motion.div
                className="absolute top-full left-0 mt-1 w-56 rounded-xl border shadow-xl z-50 overflow-hidden max-h-64 overflow-y-auto"
                style={{ background: settings.darkMode ? '#1e293b' : '#fff', borderColor: settings.darkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)' }}
                initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
              >
                {DECORATIONS.map(dec => (
      ...
      {/* ===== RULER TOP ===== */}
      {settings.showRulers && (
        <div className="shrink-0 h-5 flex items-center justify-center overflow-hidden" style={{ background: settings.darkMode ? '#1e293b' : '#e2e8f0' }}>
          <div className="h-full flex items-end" style={{ width: `calc(100% - ${settings.marginLeft + settings.marginRight}px)`, maxWidth: '210mm', margin: '0 auto' }}>
            {Array.from({ length: 40 }).map((_, i) => (
              <div key={i} className="flex-1 flex items-end justify-center">
                <div style={{ width: 1, height: i % 5 === 0 ? 10 : 5, background: i % 10 === 0 ? (settings.darkMode ? '#94a3b8' : '#64748b') : (settings.darkMode ? '#475569' : '#cbd5e1') }} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="min-h-full flex justify-center py-8">
          ...
          >
            ...
            <input
              ...
              className="w-full text-center text-2xl font-bold border-0 outline-none mb-6 bg-transparent"
              style={{
            ...
            <div
              ...
              className="outline-none min-h-[600px] arabic-text"
              style={{
                fontFamily: `'${settings.fontFamily}', serif`,
            ...
            {showFootnotes && footnotes.length > 0 && (
              <div className="mt-8 pt-4 border-t-2 border-dashed" style={{ borderColor: 'rgba(14,165,233,0.3)' }}>
                <h3 className="text-sm font-bold mb-3" style={{ fontFamily: 'Amiri, serif', color: '#1a5276' }}>
                  الهوامش
                </h3>
                {footnotes.map(fn => (
                  <div key={fn.id} className="flex gap-2 mb-2 text-[12px]" style={{ color: '#4a5568' }}>
              ...
              {clipboardItems.length === 0 ? (
                ...
                <div className="space-y-2">
                  {clipboardItems.map(item => (
                    <div
                      key={item.id}
                      className="p-2.5 rounded-lg border cursor-grab hover:bg-primary/5 transition-colors text-[11px]"
                      style={{ borderColor: settings.darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)' }}
                      onClick={() => {
                        insertHTML(item.text);
                        setShowClipboard(false);
                      }}
                      draggable
                      onDragStart={(e) => e.dataTransfer.setData('text/plain', item.text)}
                    >
                      <p className="line-clamp-3" style={{ color: textColor }}>{item.text}</p>
                      {item.source && <p className="text-[9px] text-primary/50 mt-1">{item.source}</p>}
                    </div>
                  ))}
                </div>
              )}
      ...
      {/* ===== BOTTOM STATUS BAR ===== */}
      <div className="shrink-0 border-t px-4 py-1.5 flex items-center justify-between text-[10px]"
        style={{
          background: settings.darkMode ? 'rgba(15,23,42,0.95)' : 'rgba(255,255,255,0.95)',
          borderColor: settings.darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)',
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <span>~{estimatePages(wordCount)} صفحة</span>
          <span>{footnotes.length} حاشية</span>
        </div>
        <div className="flex items-center gap-4">
          <span>الخط: {ARABIC_FONTS.find(f => f.name === settings.fontFamily)?.label}</span>
          <span>{settings.fontSize}px</span>
          <span>تباعد: {settings.lineHeight}</span>
          {lastSaved && <span>حُفظ {lastSaved}</span>}
        </div>
      </div>
    </div>
  );
