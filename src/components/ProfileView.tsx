...
export default function ProfileView() {
  ...
  // 1. تحقق من بيانات تسجيل الدخول في URL (من redirect Google)
  useEffect(() => {
  ...
  // 2. استعادة المستخدم من localStorage
  useEffect(() => {
    if (user) return; // المستخدم موجود بالفعل من URL
    try {
  ...
  // تسجيل الدخول عبر Google - يفتح صفحة Google في نفس النافذة
      ...
      <div className="flex flex-col items-center justify-center h-full px-4 py-8 max-w-md mx-auto overflow-y-auto">
        <motion.div className="w-full text-center" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <motion.button
            ...
            className="w-full flex items-center justify-center gap-3 px-6 py-3.5 sm:py-4 rounded-xl bg-card border border-border/50 hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 group"
            whileTap={{ scale: 0.97 }}
            whileHover={{ y: -2 }}
          >
